...
static int init(struct parameters *args)
{
	...
	// remove white space
#error TBSL : the final \0 is not properly dealt with
	...
	messagef(HAKA_LOG_DEBUG, L"netmap", L"links = %s", links_str);
...
#ifdef NMDEBUG
	for (idx = 0 ; idx < links_count ; idx++) {
		messagef(HAKA_LOG_DEBUG, L"netmap",
...
static void packet_verdict(struct packet *orig_pkt, filter_result result)
{
	...
	if (vbuffer_isvalid(&pkt->data)) {
		...
		if (result == FILTER_ACCEPT) {
			...
			messagef(HAKA_LOG_DEBUG, L"netmap", L"DROP");
